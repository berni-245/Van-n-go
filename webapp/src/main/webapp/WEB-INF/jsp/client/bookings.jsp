                ...
                <div class="row">
                    <div class="col-12 text-center">
                        <p class="mt-5 display-4 font-weight-bold"><spring:message
                                code="call_to_action.client_bookings"/></p>
                    </div>
                </div>
            </c:when>
            <c:otherwise>
                            ...
                            <div class="card shadow-sm mb-3">
                                <div class="card-body d-flex justify-content-between">
                                            ...
                                            <c:choose>
                                                <c:when test="${empty booking.pop or booking.pop == 0}">
                                                    ...
                                                    <form id="uploadProofOfPaymentForm_${booking.id}" method="post"
                                                          action="<c:url value='/upload/pop'/>"
                                                          enctype="multipart/form-data">
                                                        <input type="hidden" name="bookingId" value="${booking.id}">
                                                        <input type="hidden" name="driverId"
                                                               value="${booking.driver.id}">
                                                        <input type="file" id="proofInput_${booking.id}"
                                                               name="proofOfPayment"
                                                ...
                                                <c:otherwise>
                                                    <a href="<c:url value='/booking/pop?bookingId=${booking.id}' />"
                                                       target="_blank">
                                    ...
                                    <c:choose>
                                        ...
                                        <c:otherwise>
                                            <img src="${pageContext.request.contextPath}/user/pfp?userId=${booking.driver.id}"
                                                 alt="DriverPfp" class="rounded-circle"
                                                 style="width: 60px; height: 60px;"/>
                                        </c:otherwise>
                                    </c:choose>
                                </div>
